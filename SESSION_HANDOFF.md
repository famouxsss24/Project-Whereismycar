...
- **현재 상태**: 중간 발표 완료. **최종 발표용 통합 코드 작성 완료, 빌드 검증만 남음**
- **코드 위치**: `C:\wimc\wimc\` (Android 프로젝트)
- **GitHub**: `https://github.com/famouxsss24/Project-Whereismycar` (Control 브랜치 = 본인 작업)
- **빌드 상태**: ⚠️ Android Studio 빌드 직접 검증 필요 (모든 코드 작성 완료, 빌드 실패 가능성 있음)
- **다음 작업**: Android Studio에서 빌드 → 빌드 에러 있으면 수정 → Temi 설치 → 시연 리허설

---

## 🆕 최종 발표 통합 흐름 (2026-06-02 완성)

```
[1] 메인 화면 — last4 입력
[2] 차량 찾기 → Firebase 검색 (orderByChild last4)
[3] 단일/중복 처리
[4] Temi 이동 시작 (결제 없이 바로)
   ↓
[5] 네비게이션 화면 (우측 상단 박스 + 중앙 큰 글씨)
   ↓
[6] 도착 (COMPLETE 콜백)
   ↓
[7] 광고 영상 자동 재생 (jinramen_ad.mp4, 전체화면, 스킵 불가)
   ↓
[8] 광고 종료 → adWatched = true (30% 할인 확정)
   ↓
[9] 퀴즈 화면 ("류현진이 먹은 라면은?")
   ↓
[10] 정답 (진라면) → +100p 적립 / 오답 → 포인트 X (할인 유지)
   ↓
[11] 카카오페이 결제 (할인된 금액 = 원가 × 0.7)
   ↓
[12] 결제 완료 → Firebase 업데이트 → 자동 복귀
   ↓
[13] 메인 화면 복귀
```

### 핵심 변경 사항
- `paymentOptionLayout` XML은 있지만 사용 안 함 (참고용 보존)
- 결제는 도착 후에만 발생 (이동 전 결제 없음)
- 광고는 강제 시청 (스킵 불가) → 모두 30% 할인 자동 적용
- 퀴즈는 보너스 포인트 (할인과 무관)
- `entry_time` 없으면 자동 채움 (1시간 전 기준, 상지님 코드 호환 백업)
